...
Also, experiments undertaken in flow cytometry are often pilot experiments or tubes run to test and optimise panels, so not always complete datasets.
...
Processing in larger batches, or perhaps reducing the human involvment in flow cytometry, could be a first step towards automation.
...
However, as the number of samples and parameters continue to grow, and new single-cell biotechnologies such as CyTOF \citep{Becher:2014} and Drop-seq \citep{Macosko:2015} become widespread, biologists will have to relinquish more control to the computer.
