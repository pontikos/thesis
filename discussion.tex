...
In hindsight, the expected copy number group frequencies obtained from \citet{Jiang:2012cf} could have been used as prior group frequencies, by defining a Dirichlet distribution in a Bayesian setting, for all copy number groups in the clustering of the qPCR data.
%Prior probabilities come from a Dirichlet distribution.
...
Another approach to filtering outliers, as adopted by the \Rpackage{flowClust}, is to reduce the influence of outliers in parameter estimation by downweighting points by the inverse of the the Mahalanobis distance to the cluster center.  
Since the Mahalanobis distance from a point to a cluster of points is scaled by the covariance of the cluster, for a given point $\boldsymbol{x_i}$, the distance is smaller for wider clusters than for tighter clusters:

\begin{equation} \label{equation:Mahalanobis}
(\boldsymbol{x_i}-\boldsymbol{\mu})^{\top}\boldsymbol{\Sigma}^{-1}(\boldsymbol{x_i}-\boldsymbol{\mu})
\end{equation}
where $\boldsymbol{\mu}$ is the cluster mean and $\boldsymbol{\Sigma}$ is the cluster covariance.
%\[
  %u_{ig} = \frac{v+p}{v+(\boldsymbol{x_i}-\boldsymbol{\mu_g})^{\top}\boldsymbol{\Sigma}_{g}^{-1}(\boldsymbol{x_i}-\boldsymbol{\mu_g})}
%\]

It is worth noting that one issue with the Mahalanobis distance metric in detecting outliers, is that outliers influence and can greatly inflate the covariance matrix which is itself used in calculating the Mahalanobis distance.
To address this circular dependency, \gls{LOO} methods, as implemented in the \Rpackage{robustbase}, can be used to identify outliers which have high leverage on the covariance estimation.
A commonly used outlier metric in linear regression is Cook's distance, which returns the leverage of the $i^{th}$ data point on the estimation of $\hat Y$ when the point is left out:
\[
