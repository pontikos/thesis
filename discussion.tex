...
Analysing flow cytometry data has brought to light the many issues surrounding the generation and analysis of data, and more generally with the division of skill sets between generation and analysis of data.
...
The throughput is not as high as fluorescence flow cytometry, but it pushes the number of markers which can measured up to 34 and the potential of going higher.
However it cannot be used for sorting as the cells are destroyed when measured.
Also, it does not report side and forward scatter live/dead markers tend to be used instead to spot debris.
...
so their true power cannot be fully exploited as we end up having to apply hard cut-offs (see \Cref{chapter:il2ra}).
...
in a different order could perturb them.  Although I do think the hierarchical view is attractive, it easily misses populations and imposes a directionality in cell lineages which may not always be correct.
In the same way that phylogenies change as new animal species are discovered, the cell lineages should also be influenced by the discovery of new cell subsets.
...
Data quality can be as important as the methods: sophisticated the methods are no replacement for good data.
...
This prior knowledge is acquired through the experience of having seen a large number of samples and hence not encoded into an automated method.
