...
I then obtained a classification by defining a threshold on the Mahalanobis distance (\Cref{equation:Mahalanobis}), obtained from the mean and covariance of the ellipse, above which points were excluded.
...
This is probably why \contributor{Vincent Plagnol} developed his own XML parser to extract gates from FlowJo workspace files but this approach is time-consuming as it requires in-depth knowledge of the FlowJo XML schema which can change on each new release of FlowJo.
...
For example, the logarithmic transform greatly influence the identification of lower intensity populations which overlap into the negative range.
...
But as I discussed and showed in \Cref{figure:logicle-transform-w} of the \Cref{chapter:intro}, it is not always clear what transform to apply and I would argue, aiming for a Gaussian distribution is not the right criterion given the multimodality of the data.
...
With an in depth understanding of the patterns of noise, it is possible to develop more targeted approaches such as the commercial package, \texttt{ADICyt} (\url{http://www.adinis.sk/en/products/bioinformatics-and-data-processing/adicyt.html}), but these run the danger of including outlier samples.
Ultimately deciding when or not to include an outlier sample is down to a judgement call, often by the person who generated the data, so in that regard, subjectivity and bias can persist.
...
In practise, the move to automation is likely to be incremental, for example by replacing the sequence of univariate or bivariate gating steps in the process.
...
I believe we will only reap the true fruits of automated analysis once the number of samples has grown considerably, although this might require earlier steps in the process, such as the processing of samples, to be further standardised and automated first.
