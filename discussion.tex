...
These methods have the potential to be more efficient, formal, consistent, objective and better at dealing with uncertainty than manual analysis.
This could lead to improved reproducibility and more powerful association testing.
...
Ultimately the goal of normalisation and clustering is to allow for statistical association testing of genetic data with biological traits.
...
The most studied factor which improves the statistical power to detect such effects is sample size, and we are witnessing ever-increasing sample sizes as consequence of dropping genotyping and sequencing costs.
...
In most diseases, case heterogeneity is a major confounder and one which needs addressing by collection of additional clinical covariates during recruitment, to enable subgroup differentiation.

These disease phenotypes have been found in many cases to be too coarse, motivating the study of better defined quantitative intermediate phenotypes, known as endophenotypes.
It is expected that these should have stronger correlation with genotype, since they are closer to gene expression.
In my thesis, I have studied cell endophenotypes such as, CD25 protein expression on T cell subsets (\Cref{chapter:il2ra}), pSTAT5 \textit{ex vivo} response in \glspl{PBMC} (\Cref{chapter:il2}) or the relative cell subset frequency over parent cell population.
...
In \Cref{chapter:il2ra}, I assessed assessed accuracy by comparing the cluster proportions and means with those obtained from manual gating.
...
% I'd discuss the possibility of using probabilistic extensions to
% k-means, which might provide good consistency, accuracy and
% interpretability. Perhaps you already did this somewhere else in the
% thesis, but I cannot find it.
%
% A classical reference is: Bezdek, James C. (1981). Pattern Recognition with Fuzzy Objective Function Algorithms
% 
% Also: http://www1.univ-ag.fr/~rnock/Articles/Drafts/tpami06-nn.pdf
%
% Also: http://www.cs.berkeley.edu/~jordan/papers/kulis-jordan-icml12.pdf
...
It is worth noting that one issue with the Mahalanobis distance metric in detecting outliers, is that the outliers influences and greatly inflate the covariance matrix that is used in calculating the Mahalanobis distance.
...
Analysing flow cytometry data has brought to light many issues surrounding the division of skill and the difference in thinking between data generation and data analysis.
...
I found this approach gave good agreement with manual when gating CD4\positive lymphocytes on forward, side scatter and CD4 on the dataset from \Cref{chapter:il2ra}.
...
At the time of writing, a promising candidate is the \Rpackage{openCyto} which can integrate various gating algorithms to define a gating pipeline.
However, it still necessitates some programming knowledge in order to be used successfully.
% Overfull around Infinicyt
...
For example, in \Cref{chapter:kir}, qPCR allowed me to discover a SNP predictive of \gene{KIR3DS1}-\gene{KIR3DL1} copy number which we would have not found even at larger sample sizes.
