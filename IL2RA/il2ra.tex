\chapter{ \label{chapter:il2ra} The influence of gating on reproducibility and association testing in flow cytometry }
%\chapter{ \label{chapter:il2ra} Effect of \emph{IL2RA} Genotype on Cell Phenotypes }

\epigraph{ {\fontfamily{frc}\selectfont \foreignlanguage{french} { On va toujours, en fin de compte, vers où l'on pèse. }} }

\section{Background}

%\Glspl{GWAS} have found
At least two \glspl{SNP} that associate with T1D are located in the chromosome 10p15 region containing 
...
between one and six samples per day (\Cref{figure:il2ra-samples-time}).  
...
Manual gating follows the current state of knowledge of immune cell lineages and the gating strategy followed by \citet{Dendrou:2009dv} is described in \Cref{figure:il2ra-manual-gating-strategy}.  
Lymphocytes are distinguishable from more granular and larger cell types based on forward and side scatter (\Cref{figure:il2ra-manual-gating-strategy}a).
...
Within the lymphocytes, the subset expressing \protein{CD4} are defined as T lymphocytes (\Cref{figure:il2ra-manual-gating-strategy}b).
...
they express more \protein{CD127} and less \protein{CD25} than regulatory T cells (\Cref{figure:il2ra-manual-gating-strategy}c).
Non-regulatory T cells can be further divided into naive and memory subsets (\Cref{figure:il2ra-manual-gating-strategy}d).  
...
\end{minipage}
\mycaption{figure:il2ra-samples-time}
%\begin{figure}[h]
%\centering
...
\end{minipage}
\mycaption{figure:memory-CD25-MFI-time-effect}
...
\end{minipage}
\mycaption{figure:il2ra-manual-gating-strategy}
...
Within the lymphocytes a \protein{CD4}\positive subset is detected (b).
Based on expression of \protein{CD127} and \protein{CD25}, a further subset can be identified which corresponds to non-Tregs (c).
Finally, this population is divided into Memory and Naive T cells based on expression of \protein{CD45RA} and \protein{CD25} (d).
Note that the CD45RA gates in (d), excludes cells which are considered to be neither memory nor naive.
The automated gating considered in this chapter, replaces the final stage of the manual gating on \protein{CD25} and \protein{CD45RA}.
}
\end{figure}
...
instrument variation is detectable in the CD25 \gls{MFI} of the memory cell population (\Cref{figure:memory-CD25-MFI-time-effect}).
...
For example, when deciding on the position of the CD25\positive gate on the naive cell subset (\Cref{figure:il2ra-manual-gating-strategy}d), the manual gater may rely on fluorescent bead information but also on external experimental information, such as an isotype control.
...
%replace the last two univariate manual gating steps (\Cref{figure:il2ra-manual-gating-strategy}d)
...
%I will only automate the two last univariate gating steps (\Cref{figure:il2ra-manual-gating-strategy}d): 
...
%In fact extrapolating the MEF of the blank beads yields the detection threshold (\Cref{figure:flowBeads}) which we will see in the next section,
...
Applying the bead normalisation to the memory CD25 MFI from \Cref{figure:memory-CD25-MFI-time-effect}, we improve on the repeatability of that cell phenotype from $r^2=0.972$ to $r^2=0.985$, where $r^2$ is the Pearson correlation squared (\Cref{figure:CD25-MFI-beads-normalised}).
...
Also, the gate position moves down with time, reflecting the same downwards time-trend in the position of the gates observed in \Cref{figure:memory-CD25-MFI-time-effect}.
%which can be explained by position of the gates needing to be shifted down to account for this (\Cref{figure:memory-CD25-MFI-time-effect}).
...
The blue line represents our automatic CD25\positive gate which corresponds to the $86^{th}$ percentile of the blank bead population (see \Cref{figure:cd25pos-gate-agreement}).
}
\end{figure}


%As we can see from \Cref{figure:flowBeads}, the Memory T Cell MFI range is in between the MFI of the blank beads and dimmest bead population.
...
{
  Letters identify recalled individuals (\Cref{table:IL2RA-recalled-individuals}).
...
As we only have one bead set per day, we have a single fixed CD25 gate for all samples on that day (\Cref{figure:memory-CD25-MFI-time-effect}).
...
Letters are used to identify individuals, see \Cref{table:IL2RA-recalled-individuals}.
...
Overall, the association test with the percent CD25\positive naive cells phenotype yields similar effect sizes for \texttt{beads.thresh} to manual gating (\Cref{table:naive-cd25pos-association}).
