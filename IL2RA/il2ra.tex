...
To better study the downstream implications of three of these SNPs, namely \snp{rs12722495}, \snp{rs2104286} and \snp{rs11594656},
...
Manual gating follows the current state of knowledge of immune cell lineages and the gating strategy followed by \citet{Dendrou:2009dv} is described
in \Cref{figure:manual-gating-strategy}.

Part of the variation in the cell phenotypes identified with gating might be linked to covariates such genotype, age an sex.
However part of the variation is linked to instrument variation and Dendrou et al had to correct  for this with beads.
...
However the process of gating introduces variation, since gating is subjective as it relies,
to an extent, on the opinion of the gater.  
Futhermore this process is tedious on large numbers of samples.
There is therefore considerable interest in automating the process

It might worth incorporating these points into a broader discussion regarding variables in the paragraph above 
you could explain that the variables contributing to phenotypic variation of the data set are genotype, age and sex 
stating the examples you mention 
and also experimental/temporal variation as well as that introduced by gating 
which are then focusing on more...


Additionally to gating variation, long running flow experiments are also subject to instrument variation
Since samples were collected and analysed over a period of seven months, 
The variation observed through time complicates the task of gating, 
The time-dependent variation is, for example, observeable from the MFI of the memory population and this can
be corrected using beads. 
...
The bead data consist of six populations of increasing fluorescence intensity, 
...
However, even the beads populations themselves require gating, which are manually gated by \citet{Dendrou:2009dv}.
and so in the first section of the chapter, I will show how to gate the bead data automatically.
