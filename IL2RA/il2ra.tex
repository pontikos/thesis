...
and CD25 \gls{MEF} of memory cells, were found to be associated with \snp{rs2104286} and \snp{rs12722495} respectively.
...
Repeatability is an important factor, because reduced within-individual variation increases the power to detect between-individual variation.  
...
can have some bearing on the repeatability and association statistics:
%staining efficiency, 
...
Manual gating, besides being laborious on large datasets, is an inherently subjective task as it relies on the opinion of the gater \citep{Maecker:2005gm}.
...
For both the CD25 and CD45RA gating, I will be assess their performance in terms of their repeatability.
...
Since we are fitting a mixture model, instead of emulating manual gating picking a threshold,
...
Instead in the manual gating, the memory cell subset is by a threshold on the bimodal CD45RA distribution, below which cells are regarded as CD45RA\negative.
...
Although this seemed a sensible approach from the statistical perspective of fitting a two-component mixture model, it does not match the biological perspective that cells transitional should be excluded.
