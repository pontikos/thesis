...
\citet{Garg:2012jr} found that regulatory and memory \protein{CD4}\positive T cells in healthy individuals with T1D associated \gene{IL2RA} variants also
exhibit decreased sensitivity to \cytokine{IL-2} in terms of decreased MFI levels of phosphorylated \protein{STAT5} (pSTAT5),
%STAT5,
...
\citet{Long:2011hk} have reported that the \Gls{T1D} associated variant at \snp{rs1893217}
...
in part due to genetic defects in \gene{PTPN2} and \gene{IL2RA} and possibly other gene variants involved in the IL-2 signalling pathway
...
of proleukin (a polymer of \cytokine{IL-2}) at $0.1$, $10$ or \SI{1000}{\unit\per\milli\litre} respectively.
...
\end{figure}
Within each lymphocyte subset, the pSTAT5 distribution was measured per sample stimulated at each of the four increasing proleukin doses
...
\end{figure}
Next \contributor{Tony Cutler}, assessed the repeatability of the pSTAT5 response by measuring the pSTAT5 in terms of \gls{MFI}
...
effectors showing good correlation (\Cref{figure:tony-repeatability}).
...
Unfortunately, using beads on the same fluorochrome Alexa-488 used for pSTAT5 (\Cref{table:IL2-panels}),
%\Cref{chapter:il2ra}
...
\end{figure} 
On the other hand, one key issue which stands out from \Cref{figure:dose-effect-pstat5-cellsubsets-density-repeatability} is the difference in the baseline
on different days.
...
Another approach to the normalisation as implemented in the \Rpackage{flowStats}, is to align the modes of the pSTAT5 distribution across days in the ungated sample.
...
For this we use a linear transform to perfectly map the two peaks across all samples \Cref{figure:pstat5-peak-normalisation}.
...
Another metric which captures the reponse of the whole distribution across all doses instead of single dose,
