...
As seen in \Cref{chapter:il2ra}, the protective T1D associated \gene{IL2RA} variant, at \snp{rs12722495} in healthy individuals, predicts an increase in the expression of \protein{CD25}, the $\alpha$ chain of the trimeric \protein{IL-2} receptor, on memory \protein{CD4}\positive T lymphocytes \citep{Dendrou:2008gc,Dendrou:2009dv}.
...
{ Schematic of major IL-2 signaling pathway. }
{
    ...
    The pSTAT5 response to IL-2 is correlated with the relative expression of the three subunits and there is generally strong correlation between the \protein{CD25} MFI and the pSTAT5 MFI.
    %The pSTAT5 response is at its strongest when all three markers are highly expressed, intermediate when only CD122 and CD132 are expressed, and at its lowest when only CD25 is present.
%Linda: The intermediate affinity receptor is CD122 beta and CD132 gamma, not CD25 alpha and CD122 beta.
%This needs to be corrected wherever you discuss the receptor in your thesis  when only CD25 is present.  
...
%Tony:
%PTPN2 is involved in controlling other signalling pathways as well as IL-2.
...
He also included the CD3 T cell marker, as well as more NK cell specific markers, \protein{CD8} and \protein{CD56},
...
The remaining three were stimulated ex-vivo for 30 minutes at three increasing doses of proleukin, a recombinant \cytokine{IL-2 molecule}, at $0.1$, $10$ and \SI{1000}{\unit\per\milli\litre} respectively.
%Linda: Proleukin is not a polymer, it is a loose aggregate of molecules formulated this way to increase stability and bioavailability.
...
In order to control for batch, age or sex, \gls{T1D} samples were, when possible, run in parallel with an age and sex paired healthy control, although in practice, this was not always possible (\Cref{figure:IL2-sample-time}).
...
The dose-response is most striking in the smaller Treg subsets with higher \protein{CD25} (b and d).
%The dashed vertical line represents the 99th percentile of the pSTAT5 distribution in the resting sample, which is used to define the pSTAT5\positive threshold.
}
...
However, here an internal threshold was used, namely the threshold was defined to be the 99th percentile of the pSTAT5 distribution in the resting cell subset per sample.
Thus each sample and cell subset had its own pSTAT5\positive threshold.
\contributor{Tony Cutler} presented his results in six of the ten repeated individuals for the four stimulated cell subsets, memory Teffs (\Cref{figure:tony-memory-eff}), memory Tregs (\Cref{figure:tony-memory-treg}), naive Teffs (\Cref{figure:tony-naive-eff}) and naive Tregs (\Cref{figure:tony-naive-treg}).
At the highest 1000 units proleukin dose, most memory and naive Tregs are pSTAT5\positive, however, as these cell populations already show a significant response at the lowest 0.1 unit dose, the percentage of pSTAT5\positive cells at the 0.1 units dose was selected to be the relevant cell phenotype for memory and naive Treg pSTAT5 cells (\Cref{figure:tony-memory-treg,figure:tony-naive-treg}).
...
He also tested for association with \gene{IL2RA} SNP \snp{rs12722495}, and the \gene{PTPN2} SNPs \snp{rs45450798} and \snp{rs478582} (plots not shown).
...
%However, this did not appear to improve the repeatability significantly but instead just reduced the MFI by the same factor on both days (\Cref{figure:pstat5-mfi-cellsubsets-repeatability}).
...
\paragraph{\label{paragraph:ANN} Nearest-neighbour joining on whole sample}
...
but at the highest dose it appears that the least responsive naive \glspl{Teff} yield the best repeatability (\Cref{figure:repeatability-PSTAT5-MFI} and (\Cref{figure:repeatability-PSTAT5-pos}).
%Surprisingly, the memory Tregs have the highest repeatability at the 0.1 unit dose.
...
The figures are included in the Appendix \Cref{appendix:il2-association}.
...
%\includegraphics[width=\linewidth]{figures/tony-cd4-gating2}
%%\end{minipage}
%\mycaption{figure:tony-cd4-gating2}
%{ Manual gating strategy. }
%{
%Manual gating conducted using FlowJo by \contributor{Tony Cutler} to identify
%naive Teffs (green line) and Tregs (blue line) (e)
%and memory Teffs (black line) and Tregs (red line) (f).
%%The same gates can be applied across doses (i, ii, ,iii, iv).
...
\mycaption{figure:lymphocytes-all-markers}
{ Manually gated lymphocyte subsets projected on all core markers.  }
{
    ...
    Although, CD8 and CD56 are not included in the gating (\Cref{figure:tony-cd4-gating}), all the identified subsets are primarily CD8\negative and CD56\negative.
...
%Tony:
%In the total study, yes. But the paired collection strategy allows comparison on intraday samples.
...
As described in \Cref{table:IL2-panels}, a subset of samples stained with a quite comprehensive panel, including CD3, CD56 and CD8, can be used to address another clinically relevant question:
...
Biologists know that any cell which carries high levels of the three subcomponents of the IL-2 receptor, $\alpha$ (\protein{CD25}), $\beta$ (\protein{CD122}) or $\gamma$ (\protein{CD132}), should respond to IL-2, but due
%to low affinity of the CD122 antibody and
%Linda: The CD122 antibody does not have low affinity.  When methanol is used for a staining procedure, many epitopes, antibodies recognize, are destroyed (almost all are compromised as evidenced by reduced fluorescence for nearly all markers examined).  Companies are trying to optimize staining of the phosphorylated STAT5 proteins by developing antibodies to epitopes on a variety of key molecules that are retained.  Staining buffers are also being optimized.
...
%Linda: High levels of CD25 alone -- there is no signalling with IL-2.  High levels of beta alone or gamma alone -- I'm not sure if IL-2 could drive signalling with these isolated chains, maybe at very high doses--this sentence needs to be modified.  The functional signalling receptors are the trimeric (high affinity) and dimeric (Intermediate affinity -- beta/gamma) forms.  The alpha chain just increases affinity.
...
Assessing the staining quality of a sample requires prior knowledge, and so \contributor{Marcin Pekalski}, an experienced flow cytometrist, assisted me in finding a batch with no obvious staining artefacts (\Cref{figure:lymphocytes-all-markers}).
...
\end{subfigure}
\mycaption{figure:downsample}{ Example of density-dependent downsampling on bivariate 50,000 point dataset on CD45RA and CD25. }
{
...
{ Lymphocytes: (a) Mapping of cell subsets defined by manual gates onto the \gls{MST} obtained in \Cref{figure:spade-lymphocytes-pstat5mfi}.  (b) A manually identified subset of cells, \texttt{SPADE.1} (purple). }
{ (a)
...
{ The subset of cells, \texttt{lymph.SPADE.1} (purple), manually identified in \Cref{figure:spade-lymphocytes-celltypes}, is distinct from the manually gated cell subsets memory Teffs, memory Tregs, naive Teffs and naive Tregs. Its discriminating features are that it is CD4\negative, high for CD8 and CD56, while expressing low levels of CD25.
} 
%
...
{
In purple, the newly identified cell subset \texttt{lymph.SPADE.1} in \Cref{figure:spade-lymphocytes-celltypes} only shows response at 1000 units of proleukin. 
}
\end{minipage}
\end{figure}
...
While at 0.1 units little response was seen (\Cref{figure:spade-nonlymphocytes-pstat5mfi}b), two small clusters showed response at 10 units, \texttt{nonlymph.SPADE.1b} (purple) and \texttt{nonlymph.SPADE.3b} (blue), and at 1000 units, a much larger cluster highlighted in \texttt{nonlymph.SPADE.2b} (pink) (\Cref{figure:spade-nonlymphocytes-pstat5mfi}c and d).
...
I found that these three groups (purple, pink and blue) cluster around the lymphocyte scatter gate which suggested that there are no detectable clusters of cells which fall within the other major scatter clusters which constitute physically larger and more granular cells such as monocytes or granulocytes (\Cref{figure:spade-nonlymphocytes-scatter-clusters}).
The cells responsive to the lower 10 unit dose of proleukin, \texttt{SPADE.1b} and \texttt{SPADE.3b}, cluster closer to the lymphocyte scatter (\Cref{figure:spade-nonlymphocytes-scatter-clusters}a) than the large 1000 unit responsive subset of cells (in pink) (\Cref{figure:spade-nonlymphocytes-scatter-clusters}b), suggesting that the former are more likely to be lymphocytes which were not included in the manual gate.
While the larger population, \texttt{nonlymph.SPADE.2b} (pink), also tends to aggregate around the lymphocyte scatter, it further appears to aggregate in another potential, less well defined, scatter cluster, \texttt{nonlymph.SPADE.1}, delineated by the purple polygon in \Cref{figure:spade-nonlymphocytes-scatter-clusters}b.
...
{
 The three cell subsets, manually identified in the \gls{MST} of \Cref{figure:spade-nonlymphocytes-pstat5mfi} are mapped back to scatter coordinates.
...
Following further investigation of this subset of cells on other core markers in \Cref{figure:spade-nonlymphocytes-clusters}, and after filtering of doublets on side scatter width and height, while they appeared mostly CD3\positive, CD4\positive and CD56\negative therefore likely to be T cells, they also contained a small fraction of CD3\negative cells, hence likely to be a heterogeneous subset that may contain some monocytes or B cells.
%Linda: NKT cells are CD56+ so this statement isn't correct--NKT cells should be removed.
...
{
    After filtering of doublets on the side scatter height, the \texttt{nonlymph.SPADE.1} cluster defined on side and forward scatter in \Cref{figure:spade-nonlymphocytes-scatter-clusters} is displayed on the other core markers.
...
{ Non-lymphocytes pSTAT5 MFI dose-response: \texttt{nonlymph.SPADE.1}. }
{
    ...
    The black line is the lymphocyte population and the purple line is the \texttt{nonlymph.SPADE.1} subset identified in \Cref{figure:spade-nonlymphocytes-scatter-clusters}.
}
\end{minipage}
\end{figure}
...
However at 1000 units, bins which overlap with the lymphocyte cluster show clear response as well as the bin which overlaps with the uncharacterised cells, \texttt{SPADE.2b} delineated in pink in \Cref{figure:spade-nonlymphocytes-scatter-clusters}b.
...
Selecting the tree nodes manually and projecting the corresponding bins back to marker space, I plotted these two cell subsets in relation to the manually gated subsets, memory Teffs (black), memory Tregs (red), naive Teffs (green) and naive Tregs (blue) (\Cref{figure:rpart-lymphocytes-clusters}).
...
These CD56 bright cells include CD3\negative cells so could belong to a cell subset currently under investigation by \contributor{Charlie Bell} in our lab using RNAseq, which are known to express high levels of CD122, the beta chain of the IL-2 receptor.
%Linda: RPART.1 responds at 1000 U/ml--the CD56^high cells Charlie is studying respond between 1 and 10 U/ml and there is CD25 expression on a portion of the cells (at least in surface staining).  
...
{
    Two subset of cells, a 10 unit responsive subset, \texttt{lymph.RPART.2}, and a 1000 unit responsive subset, \texttt{lymph.RPART.1}, manually identified in the MST of \Cref{figure:rpart-lymphocytes-mst-1024bin}, which are distinct from the manually gated cell subsets memory Teffs (black), memory Tregs (red), naive Teffs (green) and naive Tregs (dark blue), are projected back to core marker space.
 ...
 %Linda: I would not say RPART.1 cells have high levels of CD56.
...
At 0.1 units, no clear response is visible (\Cref{figure:rpart-nonlymphocytes-mst-1024bin}b) but at 10 units (\Cref{figure:rpart-nonlymphocytes-mst-1024bin}c) certain nodes show moderate response within a branch of the MST which later become part of cluster (delineated in purple) which shows strong response at 1000 units (\Cref{figure:rpart-nonlymphocytes-mst-1024bin}d).
Projecting the cells contained within this cluster back to marker space (\Cref{figure:rpart-nonlymphocytes-scatter-clusters}), they seem to lie close to the lymphocyte cluster (black) on side and forward scatter and also appear to contain both CD4\negative and CD4\positive cells.
...
{
In order to visualise where the subset \texttt{nonlymph.RPART.1}, manually identified in \Cref{figure:rpart-nonlymphocytes-mst-1024bin}, lies in relation to the rest of the sample it is overlaid on top of the whole sample (including lymphocytes).
}
\end{minipage}
\end{figure}
...
{
    The density of the \texttt{lymph.PLSR.1} cluster (purple), identified using \gls{PLS} projection in \Cref{figure:plsr-lymphocytes},
    plotted  on all core markers, in relation to known manually gated ones within the lymphocytes.
    I only included the \texttt{lymph.PLSR.1} cluster since \texttt{lymph.PLSR.2} and \texttt{lymph.PLSR.3} do not show response to proleukin, as per \Cref{figure:plsr-lymphocytes-dose-response}.
...
In \Cref{figure:plsr-lymphocytes-clusters}, \texttt{PLSR.1} is plotted in relation to the known subset on all core markers.
...
\mycaption{figure:plsr-lymphocytes-dose-response}{ Lymphocytes pSTAT5 MFI dose response: \texttt{PLSR.1}, \texttt{PLSR.2} and \texttt{PLSR.3}. }
{
...
As can be seen in \Cref{figure:plsr-nonlymphocytes-dose-response}, of the newly identified cell subsets, only \texttt{nonlymph.PLSR.4} and \texttt{nonlymph.PLSR.5} show response from 10 units.

While this is to be expected of \texttt{nonlymph.PLSR.4} as it constitutes a subset of the lymphocytes, \texttt{nonlymph.PLSR.5} when plotted along with the lymphocytes in \Cref{figure:plsr-nonlymphocytes-clusters}, has higher side and forward scatter which suggests it constitutes an artefact or possibly another type of dose-responsive T cell, since it is CD3\positive.
...
I applied this algorithm within the lymphocyte subset, keeping the manually gated cell subsets so as to improve the two-component GMM fit and only removed them at the end once the new subsets,
\texttt{lymph.MMPART.1}, \texttt{lymph.MMPART.2} and \texttt{lymph.MMPART.3}, were identified (\Cref{figure:mmpart-lymphocytes-tree}).
These three new subsets constituted respectively, 0.21, 2.26 and 0.96 percent of the total cells in the sample.
The three subsets have different response profiles with \texttt{lymph.MMPART.1} responding to 0.1 units of proleukin, \texttt{MMPART.2} responding at 10 units and \texttt{lymph.MMPART.3} responding at 1000 units (\Cref{figure:mmpart-lymphocytes-dose-response}).
...
However, the subtle difference in CD25 MFI in the lower intensity subsets, \texttt{lymph.MMPART.1}, \texttt{lymph.MMPART.2}, memory Teffs and Tregs, does lead to suprisingly different response profiles (\Cref{figure:mmpart-lymphocytes-dose-response}).
...
I identified two subsets of cells \texttt{nonlymph.MMPART.1} and \texttt{nonlymph.MMPART.2} responsive at 0.1 and 10 units respectfully (\Cref{figure:mmpart-nonlymphocytes-tree}).
...
The \citet{Long:2010ej} study was in a relatively small number of individuals, a total of 66 cases and 125 controls, and the reproducibility of the phenotypes was not assessed.
%Tony:
%These numbers aren't correct. Where they compare Tregs between HC and T1Ds it's 15 & 17 individuals.
%They also compare responses of total CD4's in 59 control and 33 subjects.
...
%using single-cell background correction and peak normalisation,
...
%\contributor{Tony Cutler} suggests that this is because the titration of proleukin required is too fine to be performed with high accuracy so that there is a lot of uncontrolled variation in the proleukin doses.
\contributor{Tony Cutler} suggests that this is because of technical aspects of performing the proleukin stimulation experiment.
...
%Experiments done in the lab have shown when multiple aliquots of frozen stock are diluted independently on the same day, there is a variation of up to 20 pct in the measured pSTAT5 response when testing the IL-2 at limiting concentrations for a particular cell type.
...
%Since cross-batch comparisons proved unfruitful, I focused instead on identifying other, non-manually gated, dose-responsive cell subsets within a single sample, which I then tried to replicate in other batches.
...
There are other possible extensions to the regression tree methods covered in the "Elements of Statistical Learning" textbook \citep{Anonymous:ikywRZeA}.
...
An advantage of PRIM over CART is its patience, since CART tends to fragment the data rather quickly, but this comes at the cost of performance and I found the \Rpackage{prim} implementation of this method too slow to run on my dataset.
%
...
%In these scenarios, I would expect approaches which map cell subsets across samples would be more robust.
...
\citet{Ortaldo:1992tr} report that these \protein{CD3}\negative \protein{CD56}\positive cells are producers of the pro-inflammatory cytokine, interferon gamma, on stimulation with IL-2.
...
The properties and function of this \protein{CD3}\negative \protein{CD8}\negative \protein{CD45RA}\positive \protein{CD56}\positive cell subset will be further investigated in our lab by \contributor{Charlie Bell} using RNAseq.
...
\end{minipage}
\mycaption{figure:final-lymphocytes-clusters}
...
\end{minipage}
\begin{minipage}{.3\textwidth}
\mycaption{figure:final-lymphocytes-dose-response}
...
However, smaller clusters were identified, \texttt{nonlymph.SPADE.1} (\Cref{figure:spade-nonlymphocytes-scatter-clusters}) and \texttt{nonlymph.PLSR.1} (\Cref{figure:plsr-nonlymphocytes}), with larger side scatter than the lymphocytes.
...
{ Non-lymphocytes: Cell phenotypes. }
{
    ...
    %Linda: Table 3.4--in regard to the Freq listed in the table, has the debris been eliminated from the "whole sample" denominator?
}
\end{table}
...
\end{minipage}
\mycaption{figure:final-nonlymphocytes-clusters}
...
\end{minipage}
\begin{minipage}{.3\textwidth}
\mycaption{figure:final-nonlymphocytes-dose-response}
...
In the lymphocytes, I tried to identify the CD3\negative CD4\negative CD25\negative CD56\positive  subset, also known as the CD56 bright subset, corresponding to the \texttt{lymph.RPART.1} population in \Cref{figure:final-lymphocytes-clusters}.
...
The width of the lines is also increased for cells with a CD56 MFI higher than $1.2$ to better highlight the \protein{CD56}\high cells.
While, as expected, the majority of lymphocytes are \protein{CD56}\low, the \protein{CD56}\high populations clearly stand out and in particular, one line/bin shows particularly high activation.
...
{ Parallel plot of the 1024 bins obtained by using \texttt{RPART} on CD56, CD3, CD4 and CD25.  At 10 units (c), pSTAT5 MFI is already saturated in the \protein{CD25}\high bins, but a \protein{CD56}\high bin already shows moderate levels of pSTAT5 MFI.  At 1000 units (d), most bins have high levels of pSTAT5 MFI and all \protein{CD56}\positive bins show moderate pSTAT5 MFI.  }
\end{minipage}
%
\begin{minipage}{.65\textwidth}
...
Outside of the lymphocyte cluster, I attempted to identify the subset of cells with slightly larger forward scatter, that I reported as the \texttt{nonlymph.SPADE.1} subset in \Cref{figure:final-nonlymphocytes-clusters}.
...
After this filtering, the remaining cells which responded to proleukin were primarily CD4\positive CD3\positive, hence T cells, and CD45RA\positive, hence naive, as can be seen in the parallel plot representation in \Cref{figure:nonlymphocytes-singlets-KM00782Z-2012-07-23}.
...
%Linda: The SPADE.1/PLSR.1 cluster is CD3+ (figure 3.56) so these cells have to be lymphocytes -- they are falling outside of the lymphocyte light scatter gate either because they are activated or because there is a staining artefact from the intracellular protocol.  (Have doublets been removed from this analysis?)
...
For example, the \protein{CD56}\high subset
%\texttt{RPART.1}
