...
In particular, I will develop approaches of discovering biologically relevant subsets not considered by the manual gating.
...
%
\myfigure{scale=.8}{nn-pstat5-mfi-cellsubsets-repeatability}
...
%
\myfigure{scale=.8}{pstat5-pos-cellsubsets-repeatability}
...
No significant T1D association was detected with the pSTAT5 MFI (\Cref{figure:pstat5-mfi-t1d-celltypes,figure:nn-pstat5-mfi-t1d-celltypes}) nor with the percent pSTAT5\positive (\Cref{figure:pstat5-pos-t1d-celltypes,figure:nn-pstat5-pos-t1d-celltypes}) cell phenotypes in the four cell subsets considered.
...
\begin{figure}
\centering
\includegraphics[scale=.7]{figures/pstat5-mfi-t1d-celltypes}
...
As you may recall from \Cref{table:IL2-panels}, a subset of samples have been stained with a quite comprehensive panel, including CD3, CD56 and CD8, which we can use to address another clinically relevant question:
...
\begin{figure}[!h]
  \includegraphics[scale=.8]{figures/spade-lymphocytes-pstat5mfi}
...
\begin{figure}[!h]
  \includegraphics[scale=.6]{figures/spade-lymphocytes-celltypes}
...
\centering
\includegraphics[width=.7\linewidth]{figures/spade-lymphocytes-clusters}
...
{ The dashed line represents the whole sample.  The black line is the lymphocyte population and the purple line is the \texttt{SPADE.1} subset identified in \Cref{figure:spade-nonlymphocytes-scatter-clusters}.  }
\end{minipage}
\end{figure}
...
Provided that the number of bins is sufficiently large, this is conceptually another approach of reducing the number of events while preserving lower density regions,%achieving a uniform density
...
\begin{figure}[!h]
\includegraphics[width=\linewidth]{figures/rpart-lymphocytes-mst-1024bin}
...
%\begin{minipage}{\textwidth}
\centering
\includegraphics[width=.8\linewidth]{figures/rpart-lymphocytes-clusters}
...
{ Lymphocytes: The newly identified cluster using \gls{PLS} in \Cref{figure:plsr-lymphocytes} (purple) in relation to known manually gated ones within the lymphocytes. }
{
...
%\begin{minipage}{.3\textwidth}
\mycaption{figure:plsr-nonlymphocytes-clusters}
{ Non-lymphocytes: Clusters newly identified using \gls{PLS} in relation to known manually gated ones within non-lymphocytes. }
{ } 
\end{minipage}
\begin{minipage}{.5\textwidth}
...
From the pooled data, I was able to calculate multidimensional scaled representations such the \gls{MST}.  
...
This \texttt{SPADE.1}/\texttt{PLSR.1} subset contain mostly CD3\positive CD4\positive CD45RA\positive cells (\Cref{figure:final-nonlymphocytes-clusters}), thus could be activated CD4 T cells, but, according to \contributor{Marcin Pekalski} and \contributor{Tony Cutler}, could simply be an artefact of the permeabilisation and staining protocol.
...
In the lymphocytes I tried to identify the CD3\negative CD4\negative CD25\negative CD56\positive  subset, also known as the CD56 bright subset, corresponding to the \texttt{RPART.1} population from \Cref{figure:final-lymphocytes-clusters}.
...
However, instead of using the \gls{MST} visualisation, I used the parallel plot representation, where each dimension is represented as a parallel axis and each point is represented by a line segment between dimensions (\Cref{figure:lymphocytes-KM00782Z-2012-07-23}).
...
This cell population is consistently identified in four additional samples, stained with the same panel, analysed on different days, suggesting that is real and the consequence of a batch effect (\Cref{figure:lymphocytes-newsamples-cd56bright-10U}).
...
{  Lymphocytes: CD56\high cells within single batch show increased pSTAT5 MFI at 10U. }
...
\end{minipage}
\begin{minipage}{.3\textwidth}
\mycaption{figure:lymphocytes-newsamples-cd56bright-10U}
{ Lymphocytes: CD56\high cells at 10 units in four additional samples.  }
{ Parallel plot showing bins obtained by using \texttt{RPART} on CD56, CD3, CD4 and CD25.  The subset of CD56\high cells appears to exist in all samples and shows reponse at 10 units.  }
\end{minipage}
\end{figure}
...
Since I am now only interested in visualising the pSTAT5 MFI response on forward and side scatter, instead of using \texttt{RPART} as in \Cref{figure:scatter-rpart-128bin-var-pstat5} or \texttt{SPADE}, I chose to use a fixed two-dimensional grid on forward and side scatter.
...
In samples from a single batch, the subset identified previously can be visualised (delineated in purple) as it shows response at 1000 units (\Cref{figure:nonlymphocytes-KM00782Z-2012-07-23}).
This subset appears consistently in the four additional samples stained with the same panel (\Cref{figure:nonlymphocytes-newsamples-scatter}), but also in a further $49$ samples stained with the CD4 T cell panel (plots can be viewed at \url{http://goo.gl/PqJGTy}).
This suggests that it may represent a real subset of cells which are slightly larger than lymphocytes and primarily CD45RA\positive hence naive (\Cref{figure:final-nonlymphocytes-clusters}).
...
From a methodological perspective, I have also discovered the benefit of methods which makes the density more uniform so that more importance is attached to the separation between clusters rather than uniquely their weight.
