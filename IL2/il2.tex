...
{ The subset of cells, \texttt{SPADE.1} (purple), manually identified in \Cref{figure:spade-lymphocytes-celltypes}, is distinct from the manually gated cell subsets memory Teffs, memory Tregs, naive Teffs and naive Tregs. Its discriminating features are that it is CD4\negative, high for CD8 and CD56, while expressing low levels of CD25.
} 
%
...
{
In purple, the newly identified cell subset \texttt{SPADE.1} in \Cref{figure:spade-lymphocytes-celltypes} only shows response at 1000 units of proleukin. 
}
\end{minipage}
\end{figure}
...
While at 0.1 units little response was seen (\Cref{figure:spade-nonlymphocytes-pstat5mfi}b), two small clusters showed response at 10 units, \texttt{SPADE.1b} (purple) and \texttt{SPADE.3b} (blue), and at 1000 units, a much larger cluster highlighted in \texttt{SPADE.2b} (pink) (\Cref{figure:spade-nonlymphocytes-pstat5mfi}c and d).
...
While the larger population, \texttt{SPADE.2b} (pink), also tends to aggregate around the lymphocyte scatter, it further appears to aggregate in another potential, less well defined, scatter cluster, \texttt{SPADE.1}, delineated by the purple polygon in \Cref{figure:spade-nonlymphocytes-scatter-clusters}b.
...
{
After filtering of doublets on the side scatter height, the \texttt{SPADE.1} cluster defined on side and forward scatter in \Cref{figure:spade-nonlymphocytes-scatter-clusters} is displayed on the other core markers.
...
{
 Two subset of cells, a 10 unit responsive subset, \texttt{RPART.2}, and a 1000 unit responsive subset, \texttt{RPART.1}, manually identified in the MST of \Cref{figure:rpart-lymphocytes-mst-1024bin}, which are distinct from the manually gated cell subsets memory Teffs (black), memory Tregs (red), naive Teffs (green) and naive Tregs (dark blue), are projected back to core marker space.
...
{
In order to visualise where the subset \texttt{RPART.1}, manually identified in \Cref{figure:rpart-nonlymphocytes-mst-1024bin}, lies in relation to the rest of the sample it is overlaid on top of the whole sample (including lymphocytes).
}
\end{minipage}
\end{figure}
