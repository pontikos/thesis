...
{ The percentge of pSTAT5\positive cells increases with proleukin dose in memory Teffs.  }
{
...
{ Repeatability of pSTAT5\positive in six individuals. }
{
    Plot produced by Tony Cutler.
...
This, together with the improvement in repeatability observed in \Cref{chapter:il2} after bead-normalisation, motivated me to see if I could improve the repeatability of these cell phenotypes with a number of different normalisation approaches.
...
\end{subfigure}
\mycaption{figure:downsample}{ Example of density-dependent downsampling on dataset of 50,000 points on CD45RA and CD25. }
{
Effect of density-dependent (a) downsampling from 50,000 to 200 datapoints, and density-dependent (b) downsampling from 50,000 to 10 datapoints.
The small top-right cloud of points is preserved.
}
\end{figure}
...
subset, after excluding doublets.  I also excluded cells which were assigned to the manually gated subsets, both memory and naive, Teffs and Tregs, in order
...
{ Lymphocytes: First two components of \gls{PLS} projection.}
{
Clusters newly identified using \gls{PLS} in relation to known manually gated ones within lymphocytes. 
...
As can be appreciated the known cell populations tend to be better distinguishable at 0.1 units and 10 units,
while the three new clusters are better separated at 1000 units.  This is likely the consequence of the pSTAT5 response being more distinguishable
for the known subsets at 0.1 units and 10 units, while at 1000 units, the response is saturated and so undistinguishable.
...
responsive at 1000 units while the pink and light-blue subset conserve baseline pSTAT5 even at the highest dose.
...
\mycaption{figure:plsr-lymphocytes-dose-response}{ Lymphocytes: Dose response. }
{
    ...
    The pink and light-blue cluster do not respond even at the highest dose of 1000 units.
}
\end{figure}
...
{ Non-lymphocytes: Dose-response of \texttt{MMPART.1} and \texttt{MMPART.2} in relation to lymphocytes. }
{ }
\end{minipage}
\end{figure}
...
While, as expected, the majority of lymphocytes are CD56 low, the CD56 bright populations clearly stand out and in particular, one line/bin shows particular high activation.
...
All plots can be viewed at \url{http://goo.gl/PqJGTy}.
...
On one hand this cell population does appears consistently in most samples stained for pSTAT5, but on the other it is not as clearly distinguishable, based on side and forward scatter in samples which did not undergo permeabilisation for pSTAT5 staining, as can be seen in \Cref{figure:il2ra-manual-gating-strategy}.
