...
beside the cells we know of, are there other cells which respond to proleukin within the \gls{PBMC}?
...
Unfortunately, the staining using this panel was of poor quality in many samples, partly due to the permeabilisation protocol and to the larger number of markers used, which caused certain marker stains, such as CD56, to not work well across all batches.
...
flow cytometrist, assisted me in finding a batch in which the staining matched the expectation (\Cref{figure:tony-cd4-gating2}).
...
One of the challenges faced in identifying new cell types is how to reconcile the two different kinds of flow cytometric data, scatter and fluorescence, since these represent quite different physical properties and are measured on different scales, linear and logarithmic respectively.
Usually scatter gating takes precedence over fluorescence gating, and the standard is to first use scatter to discriminate live cells from debris.
...
{\gls{MST} generated by applying \gls{SPADE} on lymphocytes. \gls{MST} nodes are coloured by pSTAT5 \gls{MFI}.}
{
...
{ (a) Mapping of cell types defined by manual gates, memory Teffs (black), memory Tregs (red), naive Teffs (green) and naive Tregs (blue), to the \gls{MST} obtained
  in \Cref{figure:spade-lymphocytes-pstat5mfi}. (b) Manually identified subset of cells (purple) which respond to 1000 units but lie far from the other manually gated
  cell subsets.
}
{
  (a)
...
\myfigure{scale=.75}{spade-lymphocytes-dose-response}
{ The pSTAT5 MFI dose-response of the different cell subsets within lymphocytes. }
{
...
further appears to aggregate in another potential, less well defined, scatter cluster, delineated by the pink polygon in \Cref{figure:spade-nonlymphocytes-scatter-clusters}b.
...
\myfigure{scale=.65}{spade-nonlymphocytes-scatter-clusters}
{
...
{
  Pink cells in relation to lymphocyte cluster on all markers.
}
{
  After filtering of doublets on the the side scatter height, the pink cluster defined on side and forward scatter in \Cref{figure:spade-nonlymphocytes-scatter-clusters}
...
\myfigure{scale=.65}{spade-nonlymphocytes-dose-response}
{ Dose-response }
{
    The dashed line represents the whole sample.
    The black line is the lymphocyte population and the purple line is the new subset identified in \Cref{figure:spade-nonlymphocytes-scatter-clusters}.
}
...
Instead one could resort to \gls{PCA} or \gls{PLS} using pSTAT5 as the response variable.


\myfigure{scale=.75}{mst-marker-progression}
...
\subsection{Visualising response in whole sample with recursive partitioning} 
...
The cell subset delineated in pink constitutes around $4$ percent of the lymphocytes.
...
On the other hand, the cell subset delineated in purple appeared to be CD3\negative, CD4\positive, CD8\negative and high for CD56.
...
They constitute around $1.5$ percent of the total lymphocyte population within this sample.
...
%
\myfigure{scale=.7}{rpart-lymphocytes-mst-1024bin}
{MST built using on the 1024 bins obtained from recursive partitioning on the lymphocytes core markers.}
{
  A cluster of cells (delineated in purple) lies far from the rest of the cells which shows pSTAT5 response at 1000 units.
}

\myfigure{scale=.6}{rpart-lymphocytes-clusters}
{ A 10 unit responsive cell subset (pink) and a 1000 unit responsive cell subset (purple) within lymphocytes are identified which are not assigned to any manual gate.  }
{
  ...
  are pojected back to core marker space.
...
\myfigure{scale=.75}{rpart-lymphocytes-dose-response}
{ Dose response  }
{ }
...
\myfigure{scale=.75}{rpart-nonlymphocytes-mst-1024bin}
{MST built on the 1024 bins obtained from rpart on non-lymphocytes, a cluster of cells stands out from the rest which shows pSTAT5 response at 1000 units.}
{
}

\myfigure{scale=.65}{rpart-nonlymphocytes-scatter-clusters}
...
\myfigure{scale=.65}{rpart-nonlymphocytes-clusters}
{ A 1000 unit responsive cell subset (purple) is identified which does not belong to the manually define lymphocytes population (black).  }
{
  %In order to visualise where the points lie in relation to the rest of the sample they are are overlayed on top of a sample in which lymphocytes are present.
  %The subset of cells, delineated in purple, was manually identified in \Cref{figure:rpart-nonlymphocytes-mst-1024bin}.
...
\myfigure{scale=.5}{rpart-nonlymphocytes-dose-response}
{ A 1000 unit responsive cell subset (purple) is identified which does not belong to the manually define lymphocytes population (black).  }
{
  %In order to visualise where the points lie in relation to the rest of the sample they are are overlayed on top of a sample in which lymphocytes are present.
  %The subset of cells, delineated in purple, was manually identified in \Cref{figure:rpart-nonlymphocytes-mst-1024bin}.
...
{
  The recursive partitioning tree obtained for the pSTAT5 at 0.1 units (a), 10 units (b) and 1000 units (c) in the lymphocytes which
...
{
  The recursive partitioning tree obtained for the pSTAT5 at 0.1 units (a), 10 units (b) and 1000 units (c) in the non-lymphocyte cells.
}
{
...
A drawback of recursive partitioning techniques is that they are prone to overfitting and consequently very sensitive to batch effects.
...
%PRIM
Another extension is the \gls{PRIM} also known as the "bump hunting" algorithm
introduced by \citet{Friedman:1999iy}.
...
Since the top-down procedure is greedy at each step such an
...
\subsection{Alternative visualisation using partial least squares}
...
However the naive Teffs and naive Tregs greatly overlap and that is due to spurious correlation created by
spillover between CD45RA and FOXP3, as is apparent on the FOXP3 channel in \Cref{figure:plsr-lymphocytes-clusters},
where the naive Teffs are abnormally high in FOXP3.
...
{ First two components of \gls{PLS} projection.  Clusters newly identified using \gls{PLS} in relation to known manually gated ones within lymphocytes. }
{
...
{ The newly identified cluster using \gls{PLS} in \Cref{figure:plsr-lymphocytes} (purple) in relation to known manually gated ones. }
{
...
\myfigure{scale=.5}{plsr-lymphocytes-dose-response}
{ Dose response. }
{
...
From the first two components of the \gls{PLS} projections, I visually identified three distinct subsets in purple, light-blue and pink,
in addition to the known lymphocyte cluster in black, which were clearly discernible across all three stimulation doses.
I further identified two less discernible cell populations, a subset of the lymphocytes (orange) and a low-density cluster most visible at 10U (yellow).
As can be seen in \Cref{figure:plsr-nonlymphocytes-dose-response}, of the newly identified cell subsets, only the orange and the yellow show response
from 10U.  While this is to be expected of the orange cluster as it constitutes a subset of the lymphocytes, the yellow subset when plotted
along with the lymphocytes in \Cref{figure:plsr-nonlymphocytes-clusters}, has higher side and forward scatter which suggests it constitutes
an artefact or possibly another type of dose-responsive T cell, since it is CD3\positive.
The yellow cell subset however makes up a very small proportion of the whole sample at less that 1 percent, compared to the lymphocytes at 16 percent.

\myfigure{scale=.65}{plsr-nonlymphocytes}
{ First two components of \gls{PLS} projection.  Clusters newly identified using \gls{PLS} in relation to known manually gated ones within non-lymphocytes. }
{
    In black, the previously manually identified lymphocytes, and 
}
...
\myfigure{scale=.5}{plsr-nonlymphocytes-dose-response}
{ Dose response. }
{
...
The algorithm, as illustrated in \Cref{figure:mmpart-lymphocytes-tree}, proceeds by first dividing cells as low responders and high responders on pSTAT5 response at 1000U 
...
However, this time I kept the gated cells so as to improve the two-component GMM fit.
The manually gated cells were only removed at the end once the subsets were identified.

%
\myfigure{scale=.5}{mmpart-lymphocytes-tree}
...
%
\myfigure{scale=.65}{mmpart-lymphocytes-clusters}
{ Clusters identified from recursive partitioning. }
{ }


%
\myfigure{scale=.5}{mmpart-lymphocytes-dose-response}
{ Clusters identified from recursive partitioning. }
{ }
...
I also applied the algorithm on the non-lymphocyte subset.
Once again I left the lymphocytes in until the end to improve the model fit.
...
%
\myfigure{scale=.5}{mmpart-nonlymphocytes-tree}
{ Recursive partitioning of pSTAT5 response into low (red) and high (green) populations to identify cells responsive to the lowest dose of proleukin. }
{ }

%
\myfigure{scale=.65}{mmpart-nonlymphocytes-clusters}
{ Clusters identified from recursive partitioning. }
{ }


\myfigure{scale=.5}{mmpart-nonlymphocytes-dose-response}
{ Dose response }
{ }
...
%\begin{figure}[h]
%\centering
%\includegraphics[scale=.5]{figures/new-cell-subset2.pdf}
...
%{
%By pooling all high-responsive cells on CD4 and CD25, I identify a cluster of cells high on CD4 (a).
...
I have also tried the two main approaches of combining data across samples by either pooling or by joining.
Given the large number of events per sample in flow cytometry, pooling necessitates a way of reducing the number of events,
and I have looked at two ways of achieving this, first with density-dependent downsampling and then with binning
...
The regions were then used instead of the points in downstream analysis.
...
On the other hand, the joining did not aim to reduce the number of events but instead to normalise the number of events
across samples.
The joining was implemented using the nearest neighbour approximation in each sample to obtain a sample
containing as many events as in the base sample.
...
Colour coding the \gls{MST} by pSTAT5 response I was then able to identify new dose-responsive clusters of cells within lymphocytes and also within the general cell population which were ignored by the manual gating.
One drawback I found with considering core markers only was that each cluster and especially clusters in regions of high density,
may contain both dose-responsive and none responsive cells,
which obfuscates the identification of specific cells types.


From the joined data, I explored methods which use the pSTAT5 response to guide the binary recursive partitioning of the core marker space using regression trees.
Applying this approach to side and forward scatter, the lymphocyte population was consistently identified as the most responsive cell population in all samples stimulated at 1000 units.
...
An alternative to regression trees which avoids overfitting is random forests but needs reduction of the number of splits to be interpretable.

I found recursive partitioning to be a useful non-parametric method of exploring a dataset, in line with the tree like approach of studying flow cytometry datasets.
Unfortunately these methods are very sensitive to even small changes in the data and suffer from the biases of tree like data structures in which errors propagate.
%because of the conditional nature of the tree
...
Using this method certain a subset of highly responsive cells was identified which lie very close to the lymphocyte gate based on side and forward scatter.
The disadvantage of this approach however is since the core markers do not feature in the identification of cell subsets, there is no guarantee that the identified
...
Although spade suggests the \gls{MST} as a visualisation tool, it is not necessarily the most representative representation of the data since
...
Furthermore, the raw data used to create the \gls{MST} visualisation can been used to represent the data in a number of ways using established
\gls{MDS} methods which rely on the distance matrix computation.
...
or certain subsets not being represented.
...
One way of assessing whether samples are significantly different is to use probability binning on in the context of spade to identify clusters whose proportion
varies greatly between samples.
Generally when pooling within batches this does not seem to occur,
although \Cref{figure:scatter-rpart-128bin-var} illustrates how sensitive binning is to
small changes in the underlying data distribution 
which often result from noise, caused by debris for example,
rather than being biological significant.
%This approach across batches however will not work.
Also when the number of bins is increased it becomes much harder to detect statistically significant changes, since
we lose degrees of freedom.


Using these methods in these \gls{PBMC} samples, it is pretty conclusive that the strongest dose-response comes from within the lymphocyte subset.
...
\paragraph{Lymphocytes}
The markers and frequencies of these cell subsets identified with these methods are summarised in \Cref{table:final-lymphocytes}.
Some cell overlapped with more than one method, I merged these together into a consensus cluster.
I present in \Cref{figure:final-lymphocytes-clusters}, the cell populations which were identified by the different methods,
on all core markers, and their response in \Cref{figure:final-lymphocytes-dose-response}.
...
{ Cell phenotypes in lymphocytes. }
{ }
\end{table}

\myfigure{scale=.5}{final-lymphocytes-dose-response}
{ Dose response in lymphocytes. }
{ }


\myfigure{scale=.65}{final-lymphocytes-clusters}
{ Identified cell subset in lymphocytes. }
{ }
...
%\myfigure{scale=.65}{final-lymphocytes-clusters-consensus}
...
However a smaller cluster was consistently identified with larger side scatter than the lymphocytes.
This was found to be a heterogeneous cluster containing approximately x percent of the events in the sample.
...
{ Cell phenotypes, non-lymphocytes. }
{ }
\end{table}


\myfigure{scale=.5}{final-nonlymphocytes-dose-response}
{ Dose response in non-lymphocytes. }
{ }


\myfigure{scale=.65}{final-nonlymphocytes-clusters}
{ Identified cell subset in non-lymphocytes. }
{ }

\myfigure{scale=.65}{final-nonlymphocytes-clusters-consensus}
{ Identified cell subset consensus in non-lymphocytes. }
{ }

\clearpage


\paragraph{Correlation with CD25, CD122, CD132}

At lower doses, 0.1 and 10 units, of proleukin, CD25 explains a significant proportion of the pSTAT5 response, however at 1000 units
the response is saturated in cells with high CD25 and so the other trimeric receptors, CD122 and CD132 become more important.


\paragraph{Spillover causes spurious correlation of CD45RA and FOXP3}
