...
Furthermore, I will also consider automatic approaches for discovering biologically relevant subsets not reported by the manual gating.
...
Unfortunately, the staining of many samples using this sample was of poor quality, in part due to the permeablisation protocol,
and so certain marker stains, notably CD56, did not work across all batches.
Assessing whether staining of a sample has worked requires prior knowledge, and so \contributor{Marcin Pekalski}, an experienced
flow cytometrist, assisted me in finding a batch in which the staining looked right.
...
The classic approach, when assessing the dose-response to stimulation in a flow cytometry sample,
is first to gate cell populations in each sample based on their core markers,
...
naive and memory subsets is that they are high for CD56, CD4\negative, CD8\positive and express moderate levels of CD25.
...
Since those cells are CD8\positive CD56\positive they are NK like cells, and may have cytotoxic properties.
However they are only stimulated at a 1000 units of proleukin.
...
%(\Cref{figure:spade-nonlymphocytes-pstat5mfi}).
...
to the lymphocyte cluster (\Cref{figure:spade-nonlymphocytes-response}).  
...
or granulocytes (\Cref{figure:spade-nonlymphocytes-response}).
...
(\Cref{figure:spade-nonlymphocytes-response}a)
than the large 1000 unit responsive subset of cells (pink) (\Cref{figure:spade-nonlymphocytes-response}b),
...
further appears to aggregate in another potential, less well defined, scatter cluster, delineated by the pink polygon in \Cref{figure:spade-nonlymphocytes-response}b.

%These cells are good candidates for further investigation by further using SPADE on that scatter cluster.
Further investigation of this cell subset on other core markers in \Cref{figure:spade-nonlymphocytes-allmarkers},
they appeared mostly CD3\positive therefore likely to be T cells or NKT cells
and since they also express CD4 they look like T cells.
%However they are CD8\negative.
Also because they are bigger on forward and side scatter than lymphocytes,
they may be bigger T cell blasts, for example activated T cells, 
as they are mostly CD45RA\positive.
Also, since they contain a small fraction of CD3 negative cells,
they are likely to be a heterogeneous subset that could contain some
monocytes.
%what about CD56?
However they may well result from a technical artefact of the fixation protocol.
Further markers, possibly monoycte markers such as CD19, could be added to better define this cell type
and ascertain their clinical relevance.

\myfigure{scale=.65}{spade-nonlymphocytes-response}
{
...
\myfigure{scale=.65}{spade-nonlymphocytes-response-allmarkers}
{
  Pink cells in relation to lymphocyte cluster on all markers.
}
{
  After filtering of doublets on the the side scatter height, the pink cluster defined on side and forward scatter in \Cref{figure:spade-nonlymphocytes-response}
...
Instead one could resort to PCA or PLS using pSTAT5 as the response variable.
...
Since each bin should contain approximately $\dfrac{1}{128^{th}}$ of the events, finer binning is applied
to higher density regions and coarser binning to sparser regions.
%
...
{Proportion in sample recursively partitioned using 128 bins.}
{
...
0.1 unit and 10 unit reponsive cells is too small within each bin to influence the pSTAT5 median.
...
delineated in pink in \Cref{figure:spade-nonlymphocytes-response}b.
...
{pSTAT5 response in sample recursively partitioned into 128 bins on side and forward scatter.}
{
  ...
  Also the bin which coincides with the location of the cluster in \Cref{figure:spade-nonlymphocytes-response} shows some moderate response
...
%in \Cref{figure:scatter-rpart-127bin-var-prop}.
...
Extending recursive partitioning to all core markers, I aimed to identify dose-reponsive cells not assigned to any manual gate within the lymphocyte subset.
The number of bins was increased to 1024 and the recursive partitioning was ran on the core markers CD25, CD3, CD4, CD45RA, CD56, CD8 and FOXP3, on the lymphocyte
...
From the MST, I visually identified two repsonsive cell subsets, a 10 unit responsive one (in pink) and a 1000 unit responsive cluster (delineated in purple).
Selecting the tree nodes manually and projecting the correspondings bins back to marker space, I plotted these two cell subsets in relation to the
manually gated subsets, memory Teffs (black), memory Tregs (red), naive Teffs (green) and naive Tregs (blue) (\Cref{figure:rpart-lymphocytes-cd56bright}).
The cell subset delineate in pink constitute around $4$ of the lymphocytes.
This cell subset contains moderate levels of CD25 similar to a memory Teff and is CD3\positive, CD4\positive, CD8\negative, CD45RA\negative and FOXP3\negative.
It is likely to represent transitional cell population between memory Teffes and Tregs which was not included in the manual gating.
...
%
\myfigure{scale=.75}{rpart-lymphocytes-mst-1024bin}
{The MST was built using on the 1024 bins obtained from recursive partitioning on the lymphocytes.}
{
  A cluster of cells (delineated in purple) lies far from the rest of the cells which shows pSTAT5 response at 1000 units.
}

\myfigure{scale=.6}{rpart-lymphocytes-cd56bright}
{ A 10 unit repsonsive cell subset (pink) and a 1000 unit responsive cell subset (purple) within lymphocytes are identified which are not assigned to any manual gate.  }
{
  Two subset of cells, a 10 unit responsiece subset delineated in pink and a 1000 unit repsonive subset delineated in purple,
...
As side and forward scatter need to be included in the recursive partioning of non-lymphocytes,
...
later become part of cluster (delinated in purple) which shows strong response at 1000 units (\Cref{figure:rpart-nonlymphocytes-mst-1024bin}d).
Projecting the cells contained within this cluster back to marker space (\Cref{figure:rpart-nonlymphocytes-response}), 
...
They also overlap on side and forward scatter with the cluster identified in \Cref{figure:spade-nonlymphocytes-response}.


\myfigure{scale=.75}{rpart-nonlymphocytes-mst-1024bin}
{Building the MST on the 1024 bins obtained from rpart on the lymphocytes, a cluster of cells stands out from the rest which shows pSTAT5 response at 1000 units.}
{
}

\myfigure{scale=.75}{rpart-nonlymphocytes-response}
...
on the core markers across stimulation doses, then visually identifying clusters or bins which show pSTAT5 response using a two-dimensional \gls{MST} representation
...
However since the objective is to identify proleukin responsive cell subsets, a logical step from the methods described previously would be to use the pSTAT5 response
in guiding the clustering of these datasets. 
One way this can be achieved is to build on the recursive partitioning approach from the previous section, by using the \acrfull{CART} method,
and specifically, the regression tree and not the classification tree since the pSTAT5 response is a continuous variable.
...
This dataset can be obtained by using the \gls{ANN} algorithm introduced in \Cref{paragraph:ANN}.
...
In fact, the \gls{ANN} algorithm uses recursive partitioning technique on the core markers
...
This indexing is exploited to efficiently find the approximate nearest neighbours.
...
%Note that, contrary to the recursive partitioning scheme defined in the previous section, the tree is not necessarily balanced.
On a given ungated sample, partitioning only on side and forward scatter using pSTAT5 response at 1000 units,
I obtain the clustering in \Cref{figure:pstat5-response-decision-tree},
when the tree is pruned to the best three subsets.
This confirms that based on scatter only, the lymphocyte cluster is the most responsive cluster to proleukin.
...
as implemented in the \Rpackage{tree} fails to partition the data, since very few cells respond to lower doses of proleukin
and so the variance is not sufficiently reduced to justify a split.
...
The CART is ran on lymphocytes to see if any dose-responsive non-manually gated cell subsets are identified.
The CART is ran on the ANN joined dataset using the pSTAT5 response at each of the stimulation doses \Cref{figure:cart-lymphocytes-trees-pstat5}.
While the trees do not include all available markers in the partitioning and so the partitioning is likely to be quite coarse,
the CD56 marker features at 10 units and 1000 units.
The CART is telling us something new.
It is saying that low doses the CD25 is an important predictor,
at 10 units CD45RA\negative CD25\positive subset is the strongest responding
but the CD45RA\positive CD56\positive also responds.
In particular at the 1000 unit the highest response is in the CD3\positive subset but within the CD3\negative subset the CD56\positive
...
The CART is ran on non-lymphocytes to see if any dose-responsive cell subsets are identified.
The CART is ran on the ANN joined dataset using the pSTAT5 response at each of the stimulation doses \Cref{figure:cart-nonlymphoyctes-trees-pstat5}.
Side and forward scatter are included.
...
This illustrates a drawback of the CART and of binning in general, which is prone to overfitting and consequently generalises poorly across different samples.
...
This approach succesfully discriminates, based on side and forward scatter, the lymphocytes as the most responsive cluster
...
The gated cell were only removed at the end once the subsets were identified.


\paragraph{Non-lymphocytes}

I also applied the algorithm on the non-lymphocyte subset.
Once again I left the lymphocytes in until the end to imrove the model fit.
...
\paragraph{Extensions to regression trees}
%MARS
...
\acrfull{MARS} for example uses multiple additive regression splines
...
%PRIM
The \acrfull{PRIM} also known as the "bump hunting" algorithm
