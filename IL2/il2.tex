...
{
    \texttt{SPADE.1} subset of cells in relation to lymphocyte cluster on all markers.
}
{
...
\myfigure{scale=.75}{spade-nonlymphocytes-dose-response}
{ Dose-response }
{
...
{ Dose response of the cell subsets, \texttt{RPART.1} and \texttt{RPART.2}, within the lymphocytes in relation to the other cell subsets. }
{ }
...
{ A 1000 unit responsive cell subset (purple) is identified which does not belong to the lymphocytes population (black).  }
{
...
\myfigure{scale=.75}{rpart-nonlymphocytes-dose-response}
{ A 1000 unit responsive cell subset (purple) is identified which does not belong to the manually define lymphocytes population (black).  }
{
  %In order to visualise where the points lie in relation to the rest of the sample they are are overlayed on top of a sample in which lymphocytes are present.
  %The subset of cells, delineated in purple, was manually identified in \Cref{figure:rpart-nonlymphocytes-mst-1024bin}.
...
\myfigure{scale=.75}{plsr-lymphocytes-dose-response}
{ Dose response. }
{
...
\myfigure{scale=.75}{plsr-nonlymphocytes-dose-response}
{ Dose response of subsets identified with PLSR in relation to lymphocytes. }
{
...
This subsets constituted x, y and z percent of the total cells in the sample.
These three subsets, \texttt{MMPART.1}, \texttt{MMPART.2} and \texttt{MMPART.3}, have different response profiles with \texttt{MMPART.1} responding to 0.1 units of proleukin
...
While the high response of \texttt{MMPART.1} is to be expected, since this subset has high CD25 MFI similar to that found on memory Tregs,
it is interesting to notice that at low CD25 concentrations, the much more subtle difference in CD25 MFI between \texttt{MMPART.1} and \texttt{MMPART.2} leads to quite
different response profiles (\Cref{mmpart-lymphocytes-dose-response}).
This is also apparent in the naive and memory Teffs which have similarly low CD25 MFI but respond at 10 units.
The identified cell subsets are not homogeneous for the other markers but \texttt{MMPART.2} and \texttt{MMPART.3} appear to be CD4\negative and enriched for CD8\positive cells.
...
%
\myfigure{scale=.75}{mmpart-lymphocytes-dose-response}
...
{
    \texttt{MMPART.1} and \texttt{MMPART.2} both have forward and side scatter profiles similar to the lymphocytes.
    However \texttt{MMPART.1} also includes a peak at the top end of the side and forward scatter which are likely to represent debris.
}


\myfigure{scale=.75}{mmpart-nonlymphocytes-dose-response}
...
An alternative to regression trees which avoids overfitting is \gls{RF} but needs reduction of the number of splits to be interpretable.
...
Using this method, a subset of highly responsive cells was identified which lie very close to the lymphocyte gate based on side and forward scatter.
...
dose-responsive cells represent a homogeneous subset.
...
\gls{SPADE} contains an element of stochasticity in its downsampling so that running spade twice on the same data does not give the same tree.
...
\myfigure{scale=.75}{final-lymphocytes-dose-response}
...
\myfigure{scale=.75}{final-nonlymphocytes-dose-response}
