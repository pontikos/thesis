...
As seen in \Cref{chapter:il2ra}, the T1D associated \gene{IL2RA} variant at \snp{rs12722495} in healthy individuals predicts a decrease in expression of \protein{CD25},
...
\citet{Garg:2012jr} found that regulatory and memory \protein{CD4}\positive T cells in healthy individuals carriers of T1D associated \gene{IL2RA} variants,
also exhibit decreased sensitivity to \cytokine{IL-2} in terms of decreased MFI levels of phosphorylated \protein{STAT5} (pSTAT5),
%STAT5,
...
of \gene{FOXP3}, a transcription factor characteristic of regulatory T cells (\Cref{figure:jones-2000}).
...
of the protein tyrosine phosphatase N2 gene, \gene{PTPN2}, a negative regulatory molecule of the IL-2 pathway,
...
Hence, \contributor{Tony Cutler} set to find if he could replicate some of these findings in an independent cohort
...
as well as NK cell markers, CD3, CD8, and CD56, and the naive cell marker CD31, to discover whether we discover other potential cell subsets
also sensitive to IL-2 (\Cref{table:IL2-panels}).
...
Hence a total of 52 cases and 43 controls, of which, 11 individuals (5 cases and 6 controls) from the Cambridge Bioresource were recalled to assess reproducibility
of the phenotypes.  
...
\begin{tabular}{llll}
  \hline
Individual &  t1d & pch & difference (days) \\
  \hline
1 & 0 & a & 98 \\
2 & 1 & b & 140 \\
3 & 0 & c & 167 \\
...
However, these cell populations show a significant reponse already at the lowest 0.1 units dose of proleukin.
Based on this observation, \contributor{Tony Cutler} selected the
memory and naive Treg pSTAT5 cell phenotype to be the percentage of pSTAT5\positive at the 0.1 units dose.
On the other hand, since memory effectors are less reponsive, 10 units was chosen as the representative dose.
While for naive effectors, the less reponsive of the four cell subssets considered, the repeatability was assessed at the 1000 unit dose.
...
Tony's preliminary results in the six repeated individuals suggested that the pSTAT5 MFI and percent pSTAT5\positive phenotyes were poorly reproducible
cell phenotypes, which consequently would give us little power to detect an association with disease status or genetics.
This motivated me to see if I could improve the repeatability of these cell phenotypes using normalisation.
Furthermore, since these cell phenotypes were only defined for a single dose,
I also defined new cell phenotypes which captured the response across doses.
...
A metric which captures the reponse of the whole distribution across all doses instead of single dose,
%and not just the portion which crosses the positivity treshold,
is the area between the pSTAT5 cumulative density function in the resting sample and the stimulated sample,
this relates to the Kolomogorov-Smirnoff distance which is defined as the maximum vertical distance between cumulative density functions.
...
Provided the distribution is unimodal then the shift in the peak should correspond to the difference in the MFI.
If the distribution is not unimodal like the naive effectors then the MFI is not representative.
...
The repeatability of these metrics was tested but none were found to be repeatable neither in the gated subsets nor any of the larger subsets.
%(\Cref{XXX}).
...
Unfortunately, beads on the Alexa-488 fluorochrome, used for pSTAT5 (\Cref{table:IL2-panels}),
%\Cref{chapter:il2ra}
...
I found in practise this method to work well with a window span of $40$ across all samples considered.
...
This is particularly noteable in ungated data where debris obfuscate the pSTAT5 signal and can introduce artefactual peaks, leading to peak misidenfication.
...
a linear transform was used to perfectly map the two peaks across all samples \Cref{figure:pstat5-peak-normalisation}.
...
While using a different background MFI should account for different background resting, this assumes that the difference MFI is representative
of the pSTAT5 shift.
...
This is important because all cells do not have the same resting level of pSTAT5 (\Cref{figure:pstat5-baseline-relative}).
...
{ pSTAT5 MFI area under dose-response for each cell subset  }
{
...
{ pSTAT5\positive area under dose-response curve }
{
  The solid lines represent the median.
  The shaded areas represent the 25th and 75th percentile.
  The doses at which most of the variation occurs are 1000 units for naive effectors (green),
...
Here we will explore approaches which use the pSTAT5 response to guide the identification of cell subsets.
...
Furthermore, SPADE requires pooling samples across days for comparison of the \gls{MST}, which is not adviseable given that the core
markers are not experted to be stable across batches.
...
We find that pSTAT5 Tregs are maximally stimulated by 10 U/ml and near max at 0.1U/ml.
Whereas in their study they aren't maxed out at 100U/ml which is very odd.
One consideration is that they used frozen \gls{PBMC}, while we used fresh blood.
